...
def frequency_to_rgb(freq):
    """
    Mapeia uma frequência em Hz para uma cor RGB arbitrária com base em um espectro linear (não físico).

    Parâmetros:
    - freq: frequência dominante (Hz)

    Retorna:
    - (r, g, b): tupla com valores RGB (0-255)
    """
    import colorsys

    # Mapear faixa de frequência (por exemplo, 100Hz - 4000Hz) para faixa de matiz (0.0 - 1.0)
    min_freq = 100
    max_freq = 4000
    clamped_freq = max(min_freq, min(max_freq, freq))
    hue = (clamped_freq - min_freq) / (max_freq - min_freq)

    # Converter HSV (hue, saturação, valor) para RGB
