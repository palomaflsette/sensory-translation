...
class InstancedMeshVisual(visuals.Visual):
    def __init__(self, vertices, faces, positions, colors, transforms, subdivisions=5):
        ...
        # of instances
        self.shifts = gloo.VertexBuffer(
            positions.astype(np.float32), divisor=1)
        ...
        self.transforms_x = gloo.VertexBuffer(
            transforms[..., 0].copy(), divisor=1)
        self.transforms_y = gloo.VertexBuffer(
            transforms[..., 1].copy(), divisor=1)
        self.transforms_z = gloo.VertexBuffer(
            transforms[..., 2].copy(), divisor=1)
...
multimesh = InstancedMesh(vertices * 10, faces, pos,
                          colors, transforms, parent=view.scene)
